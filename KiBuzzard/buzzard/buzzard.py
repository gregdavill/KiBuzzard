...
from svg2mod.exporter import DEFAULT_DPI, Svg2ModExport, Svg2ModExportLatest
...
class Buzzard:
    def __init__(self):
        ...
        typeface_path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "typeface"
        )
        for entry in os.listdir(typeface_path):
            ...
            fnt_lib[os.path.splitext(os.path.basename(entry_path))[0]] = {
                "Path": entry_path
            }
    ...
    def renderLabel(self, inString):
        ...
        if (self.leftCap != "") & (self.rightCap != ""):
            pstr = "M {},{} ".format(
                bbox[0].x - alignmentOffset, bbox[0].y - padding.top
            )
            ...
            if self.leftCap == "pointer":
                pstr += "l {},{} l {},{} ".format(
                    height / -3, height / 2, height / 3, height / 2
                )
            ...
            for chunkIndex, chunk in enumerate(re.split(r"(~{.*?})", lineString)):
                ...
                if len(chunk) > 0 and chunk.isspace() == False:
                    ...
                    if chunk.startswith("~{") and chunk.endswith("}"):
                        ...
                        # Render the text
                        chunkPath.add_text(
                            chunk, origin=svg.Point(horizontalOffset, 15 * lineIndex)
                        )
                        ...
                        if self.lineOverStyle == "Square":
                            pstr = "M {},{} ".format(
                                bbox[0].x - (preSpaces * spaceWidth), bbox[0].y - 1
                            )
                            pstr += "L {},{} ".format(
                                bbox[0].x - (preSpaces * spaceWidth),
                                bbox[0].y - (self.lineOverThickness + 1),
                            )
                            pstr += "L {},{} ".format(
                                bbox[1].x + (postSpaces * spaceWidth),
                                bbox[0].y - (self.lineOverThickness + 1),
                            )
                            pstr += "L {},{} ".format(
                                bbox[1].x + (postSpaces * spaceWidth), bbox[0].y - 1
                            )
                            pstr += "z"
                            ...
                            pstr += "a {},{} 0 0 1 0,{} ".format(
                                self.lineOverThickness / 2,
                                self.lineOverThickness / 2,
                                -self.lineOverThickness,
                            )
                            pstr += "L {},{} ".format(
                                bbox[1].x + (postSpaces * spaceWidth),
                                bbox[0].y - (self.lineOverThickness + 1),
    ...
    def extractEndcaps(self, string):
        capChar = re.fullmatch(
            r"^(?P<lcap>\(|\[|\/|\\|>|<)(?P<string>.*?)(?P<rcap>>|<|\\|\/|]|\))$",
            string,
        )
...
class Svg2ModExportLatestCustom(Svg2ModExportLatest):
    ...
    def _write_library_intro(self, cmdline):
        self.output_file.write(
            """(footprint {0} (layer F.Cu) (tedit {1:8X}) (generator kibuzzard)
    ...
    """.format(
                ...
                int(
                    round(  # 1
                        os.path.getctime(self.imported.file_name)
                        if self.imported.file_name
                        else time.time()
                    )
                ),
